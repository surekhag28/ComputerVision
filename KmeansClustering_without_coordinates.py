...
# read original image and then create new image of shape (rows*cols) x 3
# each data point or pixel is represented as 3D vector
def get_data(file):
    img = cv2.imread(file)
...
def compute_new_centroids(centroids,c_labels,new_img):
    ...
    new_values = np.zeros((centroids.shape[0],4), dtype=np.float64)
